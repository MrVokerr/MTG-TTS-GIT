...
--[[Variables]]
local Deck,Tick,Test,Quality,Back=1,0.2,false,TBL.new('normal',{}),{}
Back['___']='https://steamusercontent-a.akamaihd.net/ugc/1647720103762682461/35EF6E87970E2A5D6581E7D96A99F8A575B7A15F/'
...
-- Helper to use Rikrassen's Backend (Reliable Image Host)
function RikksanSpawn(lines, qTbl, useNewEndpoint, skipTokens)
  if type(lines) == 'string' then lines = {lines} end
  if #lines == 0 then endLoop() return end
  
  local dataStr = table.concat(lines, "\n")
  -- Get card back from stored settings or default
  ...
  -- Use new endpoint as primary, old as fallback
  local endpoint = useNewEndpoint and 'https://importer.rikrassen.xyz/build' or 'https://importer-m7vpzqazfa-uc.a.run.app/build'
  ...
  WebRequest.custom(endpoint, 'POST', true, payloadJson, {
    ...
    ['X-Client-Version'] = '0.9.1'
      ...
      if wr.is_error then
        if not useNewEndpoint then
          -- Try new endpoint as fallback
          Player[qTbl.color].broadcast('Trying alternate server...', {1,0.8,0})
          RikksanSpawn(lines, qTbl, true, skipTokens)
        else
          Player[qTbl.color].broadcast('Server error: ' .. (wr.error or 'Unknown'), {1,0,0})
          endLoop()
        end
        return
      end
      ...
      for line in wr.text:gmatch("[^\r\n]+") do
         if line:find('"error"') then
            ...
            if errorCount == 1 then
               Player[qTbl.color].broadcast('Import failed for line: ' .. line, {1,0.5,0})
            end
         else
      ...
      if errorCount > 0 then
        Player[qTbl.color].broadcast('Total ' .. errorCount .. ' cards failed to import via Rikrassen Backend.', {1,0.5,0})
      end
      endLoop()
  end)
end
...
function setCard(wr,qTbl,originalData)
  if wr.text then
    local json=safeJSON(wr.text)
    if not json then
      -- If JSON fails, it might be HTML error page or empty
      Player[qTbl.color].broadcast('Error: Scryfall returned invalid data (possibly HTML error page).',{1,0,0})
      endLoop()
      return
    end
    ...
    if json.object=='card' then
      ...
      elseif json.lang=='en' then
        ScryfallGet('https://api.scryfall.com/cards/'..json.set..'/'..json.collector_number..'/'..lang,function(request)
          local locale_json = JSON.decode(request.text)
          if locale_json.object=='error' then
            Card(json,qTbl)
          else
        ...
        ScryfallGet('https://api.scryfall.com/cards/'..json.set..'/'..json.collector_number..'/en',function(a)setCard(a,qTbl,json)end)
      end
      return
...
-- replaced with a fuzzy search on the card name instead --> seems to find/get the english version after all
    elseif originalData and originalData.name then
      ScryfallGet('https://api.scryfall.com/cards/named?fuzzy='..originalData.name:gsub('%W',''),function(a)setCard(a,qTbl)end)
      return
...
function spawnDeckFromScryfall(wr,qTbl)
  ...
  for i,u in ipairs(deck)do
    Wait.time(function()ScryfallGet('https://api.scryfall.com/cards/'..u,function(c)

					local t=JSON.decode(c.text)
					if t.object~='card'then
						ScryfallGet('https://api.scryfall.com/cards/named?fuzzy=blankcard',function(c)setCard(c,qTbl)end)
...
function spawnCSV(wr,qTbl)
  ...
  for i,u in ipairs(deck)do
    Wait.time(function()
                ScryfallGet(u,function(c)
                                   local t=JSON.decode(c.text)
                                   if t.object~='card'then
                                     if u:find('&') then
                                       ScryfallGet(u:gsub('&.+',''),function(c)setCard(c,qTbl)end)
                                     else
                                       ScryfallGet('https://api.scryfall.com/cards/named?fuzzy=blankcard',function(c)setCard(c,qTbl)end)
                                     end
                                   else
      ...
      for i,card in ipairs(cards) do
        ...
        elseif card.boardType=='mainboard' or card.boardType=='commanders' or card.boardType=='companions' then
          for i=1,card.quantity do
            ...
            Wait.time(function()
              ScryfallGet('https://api.scryfall.com/cards/'..card.scryfall_id,
...
Importer=setmetatable({
    ...
    ScryfallGet('https://api.scryfall.com/cards/named?fuzzy='..qTbl.name,function(wr)
        ...
        if not obj or obj.object == 'error' then 
          -- If fuzzy fail, try search (e.g. for advanced syntax)
           ScryfallGet('https://api.scryfall.com/cards/search?q='..qTbl.name, function(wr2)
              local list = safeJSON(wr2.text)
              if list and list.data and #list.data > 0 then
                 -- For search results, spawn the first one via Rikrassen
                 local card = list.data[1]
                 local setUpper = (card.set or ""):upper()
                 RikksanSpawn("1 " .. card.name .. " (" .. setUpper .. ") " .. (card.collector_number or ""), qTbl, true, true)
              else
                 Player[qTbl.color].broadcast('Card not found: '..(obj and obj.details or "Unknown Error"),{1,0,0})
                 endLoop()
              end
           end)
           return
        ...
        if obj.object=='card' and obj.type_line:match('Token') then
           -- For Tokens, use Rikrassen with specific name
           -- Original logic did a search here, but Rikrassen likely handles "1 Token Name"
           -- Let's stick to the name Scryfall verified.
           RikksanSpawn("1 " .. obj.name, qTbl, true, true)
        else
           -- Use Rikrassen to spawn the identified card (skip tokens - just main card)
           -- We send Name, Set, CN to be precise
           local setUpper = (obj.set or ""):upper()
           RikksanSpawn("1 " .. obj.name .. " (" .. setUpper .. ") " .. obj.collector_number, qTbl, true, true)
        end 
    end)end,
          ...
          WebRequest.custom(endpoint, 'POST', true, payloadJson, {
            ...
            ['X-Client-Version'] = '0.9.1'
              ...
              if wr2.is_error then
                Player[qTbl.color].broadcast('Server error: ' .. (wr2.error or 'Unknown'), {1,0,0})
                endLoop()
                return
              end
  ...
  Print=function(qTbl)
    -- Modified to fetch prints by oracle ID if available, otherwise by name
    ScryfallGet('https://api.scryfall.com/cards/named?fuzzy='..qTbl.name, function(wr)
      local json = safeJSON(wr.text)
      if not json or json.object == 'error' then
         Player[qTbl.color].broadcast((json and json.details) or 'Card not found',{1,0,0})
         endLoop()
         return
      end
      
      local function spawnPrintsFromList(wrList)
         local list = safeJSON(wrList.text)
         if list and list.data then
             local lines = {}
             for _, card in ipairs(list.data) do
                 -- Strict Filter: unwanted types: Tokens, Emblems, Art Series, Oversized
                 local typeLine = card.type_line or ""
                 if card.layout ~= 'art_series' and 
                    card.layout ~= 'token' and 
                    card.layout ~= 'emblem' and 
                    card.layout ~= 'double_faced_token' and
                    not typeLine:find("Token") and
                    not typeLine:find("Emblem") and
                    not card.oversized then
                    
                    local setUpper = (card.set or ""):upper()
                    table.insert(lines, "1 " .. card.name .. " (" .. setUpper .. ") " .. card.collector_number)
                 end
             end
             if #lines > 0 then
                RikksanSpawn(lines, qTbl, true)
             else
                Player[qTbl.color].broadcast('No valid printings found (tokens/art series filtered).', {1,0,0})
                endLoop()
             end
         else
             Player[qTbl.color].broadcast('No printings found.', {1,0,0})
             endLoop()
         end
      end

      -- Always search by oracle_id to find all printings across sets
      if json.oracle_id then
        local printUrl = 'https://api.scryfall.com/cards/search?unique=prints&q=oracle_id%3A'..json.oracle_id
        ScryfallGet(printUrl, spawnPrintsFromList)
      else
        -- Fallback for cards without oracle ID (tokens, weird stuff)
        local printUrl = 'https://api.scryfall.com/cards/search?unique=prints&q='..qTbl.name:gsub(' ','%%20')
        ScryfallGet(printUrl, spawnPrintsFromList)
      end
    end)
  end,
  ...
  Text=function(qTbl)
    ScryfallGet('https://api.scryfall.com/cards/named?format=text&fuzzy='..qTbl.name,function(wr)
  ...
  Rules=function(qTbl)
    ScryfallGet('https://api.scryfall.com/cards/named?fuzzy='..qTbl.name,function(wr)
      ...
      elseif cardDat.object=="card" then
        ScryfallGet(cardDat.rulings_uri,function(wr)
  ...
  Booster=function(qTbl)
    ...
    if Booster[qTbl.name:upper()]then
      ...
      ScryfallGet('https://api.scryfall.com/sets/'..qTbl.name,function(w)
  ...
  Random=function(qTbl)
    ...
    if n then
      ...
      for i=1,n do
        Wait.time(function()
        ScryfallGet(url,function(wr)setCard(wr,qTbl)end)end,i*Tick)end
    else ScryfallGet(url,function(wr)setCard(wr,qTbl)end)end end,
...
function uVersion(wr)
  ...
  local s=''
  if version<v then 
    s='[77ff00]Update Ready:'..v..' Attempting Update[-]'
...
function onLoad(data)
  for _,o in pairs(getObjects())do
    local oName = o.getName()
    if oName and oName:find(mod_name) and o~=self then
  ...
  -- Initialize Back from saved data or use default
